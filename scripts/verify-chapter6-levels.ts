...
function validateLevel(level: LevelDataConfig): void {
    assert(level.chapter === 6, `关卡 ${level.id} chapter 不是 6`);
    assert(level.id >= 601 && level.id <= 611, `关卡 ${level.id} 不在 601-611 范围内`);
    ...
    assert(level.isBoss === (level.id === 611), `关卡 ${level.id} 的 Boss 标记与 id 不一致`);
}

function main(): void {
    assert(CHAPTER_6_LEVELS.length === 11, `第6章关卡数量异常：${CHAPTER_6_LEVELS.length}`);

    const chapter6Ids = CHAPTER_6_LEVELS.map(level => level.id).sort((a, b) => a - b);
    const expectedIds = Array.from({ length: 11 }, (_, index) => 601 + index);
    assert(JSON.stringify(chapter6Ids) === JSON.stringify(expectedIds), `第6章 id 覆盖不完整：${chapter6Ids.join(', ')}`);

    let previousFirstStar = 0;
    for (const level of CHAPTER_6_LEVELS) {
        validateLevel(level);
        assert(getLevelConfig(level.id) === level, `getLevelConfig(${level.id}) 未返回 chapter6 原始对象`);
        assert(level.starThresholds[0] >= previousFirstStar, `关卡 ${level.id} 难度递进异常：首星阈值未非递减`);
        previousFirstStar = level.starThresholds[0];
    }

    const chapterLevels = getChapterLevels(6);
    assert(chapterLevels.length === 11, `getChapterLevels(6) 返回数量异常：${chapterLevels.length}`);
    assert(JSON.stringify(chapterLevels.map(level => level.id)) === JSON.stringify(CHAPTER_6_LEVELS.map(level => level.id)), 'getChapterLevels(6) 顺序或映射异常');
    ...
    console.log('第6章关卡校验通过');
    console.log(JSON.stringify({
        ids: chapter6Ids,
        bossIds: CHAPTER_6_LEVELS.filter(level => level.isBoss).map(level => level.id),
        totalChapter6Levels: CHAPTER_6_LEVELS.length,
